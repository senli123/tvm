...
bool UnfoldRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  if (param->kernel_size.defined() && param->channels.defined()) {
    ...
    if (param->kernel_size.defined()) {
      ...
      if (!reporter->AssertEQ(param->kernel_size[0], wshape[2])) {
        reporter->GetDiagCtx().Emit(Diagnostic::Error(reporter->GetSpan())
                                    << "unfold: shape of weight is inconsistent with kernel_size,"
                                    << " kernel_size=" << param->kernel_size
                                    << " wshape=" << wshape);
      }
    ...
    if (param->channels.defined() && !reporter->AssertEQ(param->channels, wshape[0])) {
      reporter->GetDiagCtx().Emit(
          Diagnostic::Error(reporter->GetSpan())
          << "unfold: the first dimensions of the weight tensor (" << wshape << ")"
          << "does not match the number of channels (" << param->channels << ").");
      return false;
    }
...
TVM_REGISTER_GLOBAL("relay.op.nn._make.unfold")
    .set_body_typed([](Expr data, Expr weight, Array<IndexExpr> strides, Array<IndexExpr> padding,
                       ...
                       Array<IndexExpr> kernel_size, String data_layout, String kernel_layout,
                       String out_layout, DataType out_dtype) {
      return MakeUnfold<UnfoldAttrs>(data, weight, strides, padding, dilation, groups, channels,
                                   kernel_size, data_layout, kernel_layout, out_layout, out_dtype,
                                   "nn.unfold");
    });

RELAY_REGISTER_OP("nn.unfold")
    .describe(R"code(unfold layer (e.g. spatial convolution over images).
...
- **out**:  This depends on the `layout` parameter. Output is 4D array of shape
    ...
    .set_attr<FInferCorrectLayout>("FInferCorrectLayout", ConvInferCorrectLayout<UnfoldAttrs>)
    .set_attr<TOpPattern>("TOpPattern", kOutEWiseFusable);
